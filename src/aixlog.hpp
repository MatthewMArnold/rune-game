...
#define AIXLOG_INTERNAL__LOG_MACRO_CHOOSER(...) \
    AIXLOG_INTERNAL__VAR_PARM(                  \
        __VA_ARGS__,                            \
        AIXLOG_INTERNAL__LOG_SEVERITY_TAG,      \
...
enum class Severity : std::int8_t
{
    ...
    // info        info        INFO        INFO        INFO        SUCCESS       informational
    // message notice                  NOTICE      INFO        INFO        SUCCESS       normal, but
    ...
    //                         taken immediately EMERG system is unusable
    ...
    std::string to_string(const std::string& format = "%Y-%m-%d %H-%M-%S.#ms") const
    {
        ...
        if (pos != std::string::npos)
        {
            int ms_part = std::chrono::time_point_cast<std::chrono::milliseconds>(time_point)
                          ...
                          1000;
            char ms_str[4];
            if (snprintf(ms_str, 4, "%03d", ms_part) >= 0) result.replace(pos, 3, ms_str);
        }
        return result;
    }
    ...
    int sync() override
    {
        ...
        if (!get_stream().str().empty())
        {
            if (do_log_)
            {
                ...
                {
                    if (sink->filter.match(metadata_)) sink->log(metadata_, get_stream().str());
                }
            }
            get_stream().str("");
            get_stream().clear();
...
protected:
    virtual void do_log(std::ostream& stream, const Metadata& metadata, const std::string& message)
        const
    {
        ...
        if (pos != std::string::npos) result.replace(pos, 9, to_string(metadata.severity));
        ...
        pos = result.find("#tag");
        if (pos != std::string::npos) result.replace(pos, 4, metadata.tag ? metadata.tag.text : "");
...
struct SinkUnifiedLogging : public Sink
{
    ...
    void log(const Metadata& metadata, const std::string& message) override
    {
        os_log_with_type(
            OS_LOG_DEFAULT,
            get_os_log_type(metadata.severity),
            "%{public}s",
            message.c_str());
    }
};
...
static std::ostream& operator<<(std::ostream& os, const TextColor& text_color)
{
    ...
    if (text_color.background != Color::none) os << 39 + static_cast<int>(text_color.background);
    os << "m";
